...
n=1; 
dt = 0.1; 
n_end = 10/dt; % # of dt steps to take (time steps)
...
func1 = inline('a-u+u^2*v') %func1(a,u,v);
func2 = inline('b-u^2*v') %func2(b,u,v)
...
for n=1:n_end-1
    ...
    %du(j,n) = 0;
    %dv(j,n) = 0;
    du(j,n) = gu(j,n) + dt*func1(a, u(j,n), v(j,n));
    dv(j,n) = gv(j,n) + dt*func2(b, u(j,n), v(j,n));
    ...
    for j=2:j_end-1
        ...
        %du(j,n) = gu(j,n) + dt*func1(a,bc,uc,(3*u(j,n)-u(j-1,n))/2 , (3*v(j,n) - v(j-1,n))/2);
        du(j,n) = gu(j,n) + dt*func1(a,(3*u(j,n)-u(j-1,n))/2 , (3*v(j,n) - v(j-1,n))/2);
...
surf(v);
 ...
 %for i2=1:n_end
%     subplot(2,1,1)
%     title(['init u  ' num2str(initial_u)])
%     x1(i2) = u(:,i2);
%     y1(i2) = (i2-1)*dt;
%     %xlim([1 10])
%     hold on
